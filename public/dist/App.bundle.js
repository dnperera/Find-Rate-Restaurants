...
function autoComplete(input, latInput, lngInput) {
  ...
  var addressesDropdown = new google.maps.places.Autocomplete(input);
  addressesDropdown.addListener("place_changed", function () {
    var place = addressesDropdown.getPlace();
...
function addRemoveFavourite(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    ...
    var isTheFavourite = _this.heart.classList.toggle("heart__button--hearted");
    (0, _bling.$)(".heart-count").textContent = res.data.favourite.length;
    ...
    return console.log("Error in updating Favourites -->", err);
  });
}
exports.default = addRemoveFavourite;
...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 38;
  ...
  _axios2.default.get("/api/stores/near?lat=" + lat + "&lng=" + lng).then(function (res) {
    ...
    var markers = resturants.map(function (resturant) {
      var _resturant$location$c = _slicedToArray(resturant.location.coordinates, 2),
          resturantLng = _resturant$location$c[0],
          resturantLat = _resturant$location$c[1];

      var position = { lat: resturantLat, lng: resturantLng };
...
function searchResturant(search) {
  ...
  searchInput.on("input", function () {
    ...
    _axios2.default.get("/api/search?q=" + this.value).then(function (results) {
      if (results.data.length) {
        searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(results.data));
        return;
      }
...
exports.default = searchResturant;
...
var _searchResturant = __webpack_require__(13);

var _searchResturant2 = _interopRequireDefault(_searchResturant);

var _favourites = __webpack_require__(11);
